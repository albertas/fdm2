...
def check_assignment2(username=None):
    if not username:
        username = getpass.getuser()
    try:
        import requests
        ...
        resp = requests.post(f'https://python.lt/api/oop/2/check/{username}/', data={'objects': pickled_objects})

        if resp.ok:
            print(resp.text)
        else:
            print(f'ERROR: could not check Assignment Nr 2, because got {resp.status_code}, reason {resp.text}')

    except Exception as exc:
        print(f'ERROR: could not check Assignment Nr 2, because {exc}')
